    ...
    <app-layout :title="title">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-2xl text-gray-800 leading-tight text-center md:text-left">{{ title }}</h2>
            </div>
        </template>

        <jet-alert v-if="companyDetailsMissing && !duplicating" type="warning" class="-mt-4 mb-6">
            {{ __('Please fill out your') }}
            <Link :href="route('settings.contact_and_invoice')" class="underline hover:text-yellow-800 font-bold">{{ __('Contact and invoice data') }}</Link>
            {{ __('of your company') }}.
        </jet-alert>

        <jet-form-section @submitted="create" :with-title="false" :with-card="false">
            <template #form>
                <!-- GENERAL INFO -->
                <jet-card :title="__('General information')" class="order-1 col-span-6 lg:col-span-4 sm:pb-8">
                    <div class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            <jet-label for="inputDocumentNumber" :value="__('Invoice number')" required/>
                            <div class="grid grid-cols-2 gap-2">
                                <jet-input id="inputDocumentNumber" type="text" class="mt-1" v-model="form.number"/>
                                <jet-dropdown-select class="mt-1" :options="documentNumberOptions" v-model:value="form.invoice_number_id"/>
                            </div>
                        ...
                        <template v-for="(item, index) in form.items" :key="`item-${index}`">
                            <tr class="grid grid-cols-6 gap-x-3 gap-y-3 lg:table-row p-4 w-full">
                                ...
                                <jet-data-cell :name="__('Tax')" v-if="taxable" class="col-span-2 whitespace-nowrap align-top lg:w-[100px]" padding="lg:px-2 lg:py-4">
                                    <div class="flex rounded-md shadow-sm">
                                        <jet-numeric-input v-allow.number @input="calculateItem(item)" :min="0" :max="99" :precision="0" :empty-value="0" class="w-full rounded-r-none" v-model="item.tax" :value="(duplicating || issuingRealInvoice) ? item.tax : companyDetails.default_tax_percentage"/>
                                ...
                                <jet-data-cell colspan="5" class="lg:px-2 align-top" :padding="null">
                                    ...
                                    <div v-if="item.discountToggled" class="grid grid-cols-11 gap-4 m-0 font-semibold bg-green-50 ring-2 ring-green-100 p-3 rounded-md h-16 mb-4 lg:max-w-[515px]">
                                        <div class="col-span-3">
                                            <div class="flex rounded-md shadow-sm">
                                                <jet-numeric-input v-allow.number.comma @input="calculateDiscount('percentage');calculateItem(item)" :min="0" :precision="selectedCurrency.precision" :empty-value="0" separator="." class="w-full rounded-r-none" v-model="item.discount_percent" :value="item.discount_percent"/>
                ...
                <jet-card id="invoice-summary" class="order-4 md:order-5 col-span-6 md:col-span-3 lg:col-span-2 overflow-hidden">
                    <dl class="text-right border-r-8 border-indigo-500 -m-6">
                        <div v-if="taxable" class="text-base md:text-sm odd:bg-gray-50 px-4 py-5 grid grid-cols-2 gap-4 px-6">
                        ...
                        <div class="text-xl md:text-lg odd:bg-gray-50 px-4 py-5 grid grid-cols-2 gap-4 px-6">
                            ...
                            <dd class="mt-1 font-bold text-gray-900 mt-0 col-span-1">
                                {{ taxable ? money(form.amount_with_tax) : money(form.amount) }}
                            </dd>
                        </div>
                    </dl>
                </jet-card>
            ...
            <template #actions>
                ...
                <jet-button @click="downloadPdf = false" :class="{'opacity-25': form.processing}" :disabled="form.processing || companyDetailsMissing" class="justify-center px-5 h-10">
                    <document-add-icon class="w-4 h-4 -ml-1 mr-2"/>{{ __('Save') }}
                </jet-button>

                <jet-button @click="downloadPdf = true" :class="{'opacity-25': form.processing}" :disabled="form.processing || companyDetailsMissing" class="justify-center px-5 h-10 ml-3 hidden sm:flex">
        ...
        <missing-bank-accounts-warning :show="modal.warningBankAccount" @close="modal.warningBankAccount = false"/>
    </app-layout>
</template>
...
<script>
    ...
    import JetAlert from '@/Jetstream/Alert.vue'
    ...
    import MissingBankAccountsWarning from "@/Pages/Modals/Client/MissingBankAccountsWarning"
    ...
    import CreateDocumentWatchers from "@/Pages/Documents/Shared/Watchers/CreateDocumentWatchers";

    export default defineComponent({
        name: 'CreateInvoice',

        mixins: [DocumentMixin, CreateDocumentWatchers],
        ...
        props: [
            'document', 'type', 'companyDetails', 'bankAccounts', 'clients', 'documentCompanyName', 'isTaxPayer',
            'companyDetailsMissing', 'documentNumberOptions', 'countryDropdownItems', 'issuingRealInvoice', 'userDefaults',
        ],
        ...
        data() {
            return {
                form: this.$inertia.form({
                    ...
                    tax: 0,
                    discount: 0,
                    creating_from_document_id: null,
                    date_created: moment().format('YYYY-MM-DD'),
                    date_added: moment().format('YYYY-MM-DD'),
                    billing_date: moment().add(14, 'days').format('YYYY-MM-DD'),
                    variable_symbol: '',
                    constant_symbol: '',
                    specific_symbol: '',
                    order_number: '',
                    ...
                    note_before_items: '',
                    client_id: null,
                    issued_by: this.userDefaults?.issuedBy?.name ? this.userDefaults.issuedBy.name : this.$page.props.user.name,
                    issued_by_phone: this.userDefaults?.issuedBy?.phone ? this.userDefaults.issuedBy.phone : '',
                    issued_by_email: this.userDefaults?.issuedBy?.email ? this.userDefaults.issuedBy.email : this.$page.props.user.email,
                    taxable: null,
                    lang_code: this.$page.props.locale.current,
                    ...
                    items: [{
                        ...
                        price_with_tax: 0,
                        tax: this.companyDetails.default_tax_percentage,
                        calculated_from_subtotal: null,
                        quantity: 1,
                        unit: 'ks',
                        image: null,
                        discount_amount: 0,
        ...
        computed: {
            title() {
                if (this.issuingRealInvoice) {
                    return this.__('Issuing real invoice') + ': ' + this.document.number;
                }
                return this.duplicating
                    ? this.__('Duplicating invoice') + ': ' + this.document.number
                    : this.__('New invoice');
            },
        ...
        mounted() {
            this.modal.warningBankAccount = this.bankAccounts.length === 0 && !this.duplicating;
            ...
            if (this.duplicating) {
                ...
                this.form.client_id = route().params.client;
                ...
                this.form.billing_data.company_details = this.document.company_details;
                this.form.taxable = this.taxable;

                if (this.issuingRealInvoice) {
                    // ISSUING REAL INVOICE
                    this.form.creating_from_document_id = this.document.id;
                    this.form.note = this.document.note ? this.document.note +'\n' : '';
                    this.form.note += this.__('Created from proforma invoice') +' '+ this.document.number;

                    this.form.items = this.document.items.map(item => {
                        item.tax = this.taxable ? this.companyDetails.default_tax_percentage : 0;
                        return item;
                    });
                } else {
                ...
                const defaultDocumentNumber = this.documentNumberOptions.find(i => i.is_default);
                this.form.invoice_number_id = defaultDocumentNumber?.value;

                this.form.items = this.document.items.map(item => {
                    item.discountToggled = item.discount_amount > 0;
                ...
                const defaultDocumentNumber = this.documentNumberOptions.find(i => i.is_default);
                this.form.invoice_number_id = defaultDocumentNumber?.value;
                this.form.billing_data.with_logo =  this.companyDetails != null ? this.companyDetails.logo !== null : false;
                this.form.billing_data.bank_accounts = cloneDeep(this.bankAccounts);
                this.form.billing_data.company_details = cloneDeep(this.companyDetails);
        ...
        methods: {
            create() {
                ...
                const url = this.issuingRealInvoice
                    ? route('documents.issue.real_invoice.store')
                    : route('documents.store', this.type);
                ...
                this.form.post(url, {
                    onSuccess: () => {
                        ...
                        if (this.downloadPdf) {
                            return axios.get(route('documents.download_latest', this.type), {responseType: 'blob'})
                                .then(response => {
