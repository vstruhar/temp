<template>
    <app-layout :title="__('Proforma invoice')+': '+document.number">
        <template #header>
            <h2 class="font-semibold text-2xl text-gray-800 leading-tight text-center md:text-left">
                {{ __('Proforma invoice') }}: {{ document.number }}
            </h2>
        </template>
                    ...
                    <template #form>
                        ...
                        <div class="col-span-6 mt-6">
                            ...
                            <table class="divide-y divide-gray-200 -mt-4 w-full border-b">
                                ...
                                <tr>
                                    ...
                                    <jet-header-cell class="px-2 py-3">{{ __('Total') }}</jet-header-cell>
                                </tr>
                                </thead>
                                ...
                                <template v-for="(item, index) in document.items" :key="`item-${index}`">
                                    <tr class="grid grid-cols-6 lg:table-row gap-y-2 sm:gap-y-0 p-4 w-full even:bg-gray-50 lg:even:bg-transparent">
                                        ...
                                        <jet-data-cell :name="__('Total')" v-else padding="lg:px-2 lg:py-4" class="col-span-3 sm:col-span-2 min-w-[100px] max-w-[120px]">
                                            {{ money((item.price * item.quantity) - item.discount_amount) }}
                                        </jet-data-cell>
                                    </tr>
                                </template>
                                </tbody>
                        ...
                        <jet-section class="col-start-1 md:col-start-3 lg:col-start-4 col-end-7 mt-2 mb-2 overflow-hidden">
                            <dl class="text-right border-r-8 border-indigo-500 -m-6">
                                <div v-if="hasDiscount" class="text-xl md:text-lg bg-gray-50 px-4 py-5 grid grid-cols-2 gap-4 px-6">
                                    <dt class="text-sm font-medium text-gray-500">{{ __('Total discount') }}</dt>
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">{{ money(totalDiscount) }}</dd>
                                </div>
            ...
            <div class="col-span-9 lg:col-span-2 grid grid-cols-6 lg:block gap-x-0 sm:gap-x-6 lg:gap-x-0 mb-5">
                ...
                <jet-section class="mb-5 col-span-6 sm:col-span-3 lg:col-span-6" no-padding>
                    <div class="divide-y divide-gray-200">
                        <button v-if="can('documents:payments:create')"
                                ...
                                class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition"
                        ...
                        <button v-if="can('documents:proforma_invoice:print')" @click="printPdf" :disabled="loadingDocumentPdf" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition">
                            <printer-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Print') }}
                        </button>

                        <button v-if="can('documents:proforma_invoice:download')" @click="downloadPdf" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition">
                            <document-download-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Download') }}
                        </button>

                        <Link v-if="can('documents:proforma_invoice:send')" :href="route('documents.send_form', ['proforma-invoices', document.id])" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition">
                            <mail-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Send') }}
                        </Link>
                    </div>
                </jet-section>

                <jet-section class="mb-5 col-span-6 sm:col-span-3 lg:col-span-6" v-if="can('documents:proforma_invoice:edit') || can('documents:proforma_invoice:duplicate') || can('documents:proforma_invoice:delete')" no-padding>
                    <div class="divide-y divide-gray-200">
                        <Link v-if="can('documents:proforma_invoice:edit')" :href="route('documents.edit', ['proforma-invoices', document.id])" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition">
                            <pencil-alt-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Edit') }}
                        </Link>

                        <button v-if="can('documents:proforma_invoice:duplicate')" @click="duplicate" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition">
                            <document-duplicate-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Duplicate') }}
                        </button>

                        <button v-if="can('documents:proforma_invoice:delete')" @click="modal.confirmDeleteDocument = true" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-red-500 hover:text-red-700 hover:bg-red-100 transition">
                            <trash-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Delete') }}
                        </button>
                    </div>
                </jet-section>

                <jet-section class="mb-5 col-span-6 sm:col-span-3 lg:col-span-6" v-if="document.invoice_id">
                    <div class="-m-6 divide-y divide-gray-200">
                        ...
                        <Link v-if="can('documents:invoice:show')" :href="route('documents.show', ['invoices', document.invoice_id])" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition">
                            <document-text-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Real invoice') }}
                        </Link>
                    </div>
                </jet-section>

                <jet-section class="mb-5 col-span-6 sm:col-span-3 lg:col-span-6" v-if="!document.invoice_id && can('documents:invoice:create')">
                    <div class="-m-6 divide-y divide-gray-200">
                        <div class="text-sm tracking-wider font-semibold text-gray-400 py-2 text-center bg-gray-50 rounded-t-lg">{{ __('Issue') }}</div>

                        <Link v-if="can('documents:invoice:send')" :href="route('documents.issue.real_invoice.create', document.id)" class="flex items-center px-5 py-4 w-full text-base tracking-wide text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition">
                            <document-add-icon class="w-5 h-5 ml-1 mr-5"/> {{ __('Real invoice') }}
                        </Link>
                    </div>
                </jet-section>
                ...
                <jet-section class="col-span-6 sm:col-span-3 lg:col-span-6" v-if="document.revisions.length && can('documents:revision:restore')" classes="p-0">
                    <history-of-changes :revisions="document.revisions"
                                        :invoice-id="document.id"
                                        :title="__('Restore proforma invoice')"
                                        :message="__('Are you sure? This action will restore proforma invoice to selected version.')"
                                        :confirm-button-text="__('Restore proforma invoice')"/>
                </jet-section>
            </div>
        </div>
        ...
        <duplicate-document-modal type="proforma-invoices"/>
    </app-layout>
</template>
...
<script>
    ...
    import {InformationCircleIcon, PlusIcon, DocumentTextIcon} from '@heroicons/vue/outline'
    import {Link} from '@inertiajs/inertia-vue3'
    import {PencilAltIcon, TagIcon, DocumentAddIcon, DocumentDownloadIcon, MailIcon, DocumentDuplicateIcon, TrashIcon, PrinterIcon} from '@heroicons/vue/solid'
    ...
    export default defineComponent({
        ...
        computed: {
            ...
            totalDiscount() {
                return this.document.items.reduce((total, i) => total + i.discount_amount, 0);
            },
        ...
        methods: {
            ...
            downloadPdf() {
                axios.get(route('documents.download', ['proforma-invoices', this.document.id]), {responseType: 'blob'})
                    .then(response => {
            ...
            deleteDocument() {
                this.$inertia.form().delete(route('documents.delete', ['proforma-invoices', this.document.id]), {
                    onError: handleError,
                });
            },
