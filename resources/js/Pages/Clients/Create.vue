<template>
    <app-layout :title="__('Add client')">
        <template #header>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-0">
            <jet-form-section @submitted="saveClient" :with-title="false" :with-card="false">
                <template #form>
                    <jet-card :title="__('Contact and invoice data')" class="col-span-6">
                        <!-- Name -->
                        <div class="w-full mb-7">
                            <jet-label :value="__('Name')" required/>
                            <jet-company-autocomplete class="mt-1" v-model:value="form.name" :country="form.country" @companySelected="populateFields"/>
                            <jet-input-error :message="form.errors.name" class="mt-2"/>
                        </div>

                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6 lg:col-span-2 grid grid-rows-4 gap-5 border-gray-100 border-b-4 lg:border-r-4 md:border-b-0 lg:pr-6 pb-7 md:pb-0">
                                <!-- Address -->
                                <div>
                            ...
                            <jet-table class="border-none -mt-3">
                                ...
                                <template #tbody>
                                    <jet-row v-for="(contact, index) in form.contacts" :key="`contact-table-row-${contact.id}`" class="cursor-auto">
                                        <jet-data-cell :name="__('Name')" class="col-span-2 px-0 py-0 lg:px-4 lg:py-5" slot-classes="w-full lg:pl-2">
                                            <jet-input type="text" class="w-full" v-model="contact.name"/>
                                        </jet-data-cell>

                                        <jet-data-cell :name="__('Email')" class="col-span-2 px-0 py-0 lg:px-4 lg:py-5" slot-classes="w-full">
                                            <jet-input type="text" class="w-full" v-model="contact.email"/>
                                        </jet-data-cell>

                                        <jet-data-cell :name="__('Phone')" class="col-span-2 px-0 py-0 lg:px-4 lg:py-5" slot-classes="w-full">
...
export default defineComponent({
    ...
    methods: {
        ...
        saveClient() {
            this.form.post(route('clients.store'), {onError: handleError});
        },
