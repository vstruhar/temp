<template>
    <modal :show="show" max-width="4xl" :closeable="true" :with-card="false" @close="$emit('close')">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-0">
            <div class="bg-white overflow-hidden shadow-md sm:rounded-lg">
                <jet-form-section @submitted="createClient" :with-title="false">
                    <template #form>
                        <div class="col-span-6 bg-gray-50 -m-6 mb-0 p-5 flex justify-between rounded-t-lg">
                            <div class="font-semibold text-2xl text-gray-800">{{ __('Add client') }}</div>
                        </div>
    ...
    export default defineComponent({
        ...
        methods: {
            createClient() {
                ...
                axios.post(route('clients.store'), this.form)
                    .then(({data}) => {
